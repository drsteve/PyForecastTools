...
class contingency2x2_construction(unittest.TestCase):
    ...
    def test_fromBool_arrays(self):
        '''Check construction from Boolean series'''
        ctab = verify.Contingency2x2.fromBoolean(np.asarray(self.predlist),
                                                 np.asarray(self.obslist))
        npt.assert_array_equal(self.res, ctab)
    ...
    def test_direct_array(self):
        '''check construction direct from rates as array'''
...
class contingency2x2_calc(unittest.TestCase):
    ...
    def test_majorityClass_pos(self):
        '''Test estimate of majority classifier (uses PC) for True majority'''
        ctab = verify.Contingency2x2.fromBoolean(self.predlist, self.obslist)
        self.assertEqual(ctab.majorityClassFraction(), self.MC)

    def test_majorityClass_neg(self):
        '''Test estimate of majority classifier (uses PC) for False majority'''
    ...
    def test_oddsRatio_known(self):
        '''
        Test Odds ratio calculation with known example

        Example per Thornes and Stephenson
        '''
    ...
    def test_YuleQ_known(self):
        '''
        Test Yule's Q (ORSS) calculation with known example
    ...
    def test_Finley_PSS_CIWald_regress(self):
        ctab = verify.Contingency2x2(self.Finley)
        npt.assert_array_almost_equal(ctab.peirce(ci='Wald'),
                                      [0.5228568171454651, 0.13669651496274715],
                                      decimal=5)

    def test_Finley_PSS_CIAgresti_regress(self):
        ctab = verify.Contingency2x2(self.Finley)
        npt.assert_array_almost_equal(ctab.peirce(ci='AC'),
                                      [0.5228568171454651, 0.13187940039376933],
                                      decimal=5)
    ...
    def test_Finley_MC(self):
        '''Ensure that PC for constant Finley equals MC for standard Finley'''
        ctab = verify.Contingency2x2(self.Finley)
    ...
    def test_BootstrapCI_regress(self):
        '''Test bootstrap CI calculation gives expected answer'''
...
class contingencyNxN_tests(unittest.TestCase):
    def test_NxNto2x2(self):
        '''
        Goldsmith's 3 category non-probabilistic forecasts

        [see Wilks, 1995, p273]
        '''
        tt = verify.ContingencyNxN([[50, 91, 71],
                                    [47, 2364, 170],
                                    [54, 205, 3288]])
        tt2 = tt.get2x2(0)
...
class perfectContinuous(unittest.TestCase):
    ...
    def setUp(self):
        super(self.__class__, self).setUp()
        self.predvec = [8, 9, 11, 10, 10, 11, 15, 19, 17, 16, 12,
                        8, 6, 7, 9, 15, 16, 17, 24, 28, 32, 28, 20]
    ...
    def test_percBetter(self):
        '''
        Percent Better of perfect prediction

        PB of perfect forecast relative to all-incorrect is 100
        '''
        badmodel = verify.metrics._maskSeries(self.predvec) - 1
        self.assertEqual(verify.percBetter(self.predvec,
                                           badmodel,
                                           self.obsvec), 100)
    ...
    def test_logAccuracy(self):
        ...
        npt.assert_array_equal(verify.logAccuracy(self.predvec,
                                                  self.obsvec,
                                                  mask=False), ans)
...
class relativeContinuous(unittest.TestCase):
    ...
    def test_APE2(self):
        ...
        ans = np.ones_like(self.obsvec)
        npt.assert_array_equal(verify.absPercError(np.asarray(self.predvec)*-1,
                                                   np.asarray(self.obsvec)*-1),
                               ans*70)

    def test_meanPercentageError(self):
        '''Test estimation of mean percentage error'''
        self.assertEqual(verify.meanPercentageError(self.predvec,
                                                    self.obsvec),
                         70)
...
class otherContinuous(unittest.TestCase):
    ...
    def test_RMSE_offset(self):
        '''
        Test that RMSE is correct for constant prediction offset

        That is, prediction is a constant offset from constant observation
        '''
        val, diff = 5, 2
        n_elements = 12
        predvec = [val]*n_elements
    ...
    def test_RMSE_offset_neg(self):
        ...
        I.e., constant offset from constant observation (with negatives)
        '''
        val, diff = -5, -2
        n_elements = 12
        predvec = [val]*n_elements
    ...
    def test_medAbsError_equal_meanAbsError_const(self):
        '''med abs error equals mean abs error for constant pred/obs vectors'''
        val, diff = 5, 2
        n_elements = 12
        predvec = [val]*n_elements
        obsvec = [val-diff]*n_elements
        npt.assert_array_equal(verify.medAbsError(predvec, obsvec),
                               verify.meanAbsError(predvec, obsvec))

    def test_medSymAcc_AllMethodsEqual1(self):
        '''median symmetric accuracy is within precision for diff methods'''
        np.random.seed(24601)
        predvec = np.arange(1, 40)
        obsvec = predvec+np.random.random(len(predvec))
        npt.assert_array_almost_equal(verify.medSymAccuracy(predvec, obsvec),
                                      verify.medSymAccuracy(predvec, obsvec,
                                                            method='log'))
...
class badInputs(unittest.TestCase):
    ...
    def test_medSymAcc_raises(self):
        '''median symmetric accuracy raises an error with invalid method'''
        self.assertRaises(NotImplementedError,
                          verify.medSymAccuracy,
                          self.predvec, self.obsvec,
                          method='NotAMethod')
    ...
    def test_logAcc_negative_raises(self):
        ...
        self.assertRaises(ValueError, verify.logAccuracy,
                          self.predvec*-1, self.obsvec,
                          mask=False)

    def test_logAcc_negative_noRaiseWhenMasked(self):
        '''log accuracy masks all values when predicted array is all negative'''
        with warnings.catch_warnings():
            # squelch RuntimeWarnings as this will raise w/ msg
            # "invalid value encountered in log10"
            warnings.simplefilter("ignore")
            self.assertTrue(verify.logAccuracy(self.predvec*-1,
                                               self.obsvec,
                                               mask=True).mask.all())

    def test_BootstrapCI_failsWithoutBooleanCreate(self):
        '''Test bootstrap CI raises error without input booleans

        When a table is created without input booleans the bootstrap
        CI should raise an error
        '''
        ctab = verify.Contingency2x2(self.Finley)
...
class precisionMeasures(unittest.TestCase):
    ...
    def test_Sn_expected_Gaussian_even(self):
        '''
        Test that Sn estimator has a mean estimated value
    ...
    def test_Sn_masked_value(self):
        '''Test that Sn estimator doesn't hit errors/warnings with badvals'''
